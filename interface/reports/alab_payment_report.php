...
if (!AclMain::aclCheckCore('acct', 'rep_a')) {
    echo (new TwigContainer(null, $GLOBALS['kernel']))->getTwig()->render('core/unauthorized.html.twig', ['pageTitle' => xl("alab payment Report")]);
    exit;
}
...
if (!empty($_POST['download_csv'])) {
    ...
    if ($form_date) {
        ...
        if ($form_to_date) {
            $where .= "s.created_time >= ? AND s.created_time <= ? ";
    ...
    $query = "SELECT a.pid, a.encounter as encounter_id,CONCAT(pd.lname, ', ', pd.fname) AS patient_name,i.name as Insurance_Name,s.payment_method,s.reference,a.memo,a.pay_amount, a.adj_amount, a.post_time, 
            a.session_id, a.code, a.payer_type, fe.id, fe.date, fe.provider_id, fe.id, 
         fe.invoice_refno, s.deposit_date, s.payer_id    
          FROM ar_activity AS a 
          JOIN patient_data pd on a.pid=pd.pid
          ...
          WHERE a.deleted IS NULL AND (a.pay_amount != 0 OR a.adj_amount != 0) and " .
        $where ;
       
    $eres = sqlStatement($query, $sqlArray);

    $filename = "alab_payment_report_export_{$timestamp}.csv";
...
?>
    ...
    <head>
        ...
        <title><?php echo xlt('Alab Payment Report') ?></title>
    ...
    <body class="body_top">
        ...
        <span class='title'><?php echo xlt('Report'); ?> - <?php echo xlt('ALAB Payment Report'); ?></span>

        <form method='post' action='alab_payment_report.php' enctype='multipart/form-data' id='theform' onsubmit='return top.restoreSession()'>
                            ...
                            <table>
                                ...
                                <tr>
                                    <td class='col-form-label'>
                                        <?php echo xlt('Payment Create From Date'); ?>:
                                    </td>
                                    <td>
                                    ...
                                    <td class='col-form-label'>
                                        <?php echo xlt('Payment Create To Date {{Range}}'); ?>:
                                    </td>
                                    <td>
